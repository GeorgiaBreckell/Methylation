...
motif_plot<-ggplot(df,aes(x=Pval,y=(1:length(Pval)/length(Pval)) ))+
  ...
  xlim(-60,0)+
  labs(title = paste(filenames," ", motif,"first million sites unfiltered"), x="T test P value", y= "")
...
filtered_rds<-unfiltered_rds%>%
  mutate(u_test_pval_log=log10(u_test_pval))%>%
  mutate(passed_filter=if_else(u_test_pval_log<pval_cutoff,1,0))
...
motifs_sites_clean<-motif_sites%>%
  ...
  #mutate(Contig=gsub("contig_4_pilon","contig_4",Contig))%>%
  drop_na(Site)%>%
  add_column(Site_mod=0)%>%
  add_column(Pval=0)%>%
...
for (i in 1:length(motifs_sites_clean$Site)) {
  ...
  pos.mod<-subset(filtered_rds, position < (motifs_sites_clean[i,2]+ window) & position > (motifs_sites_clean[i,2]-window) & contig == motifs_sites_clean[i,1])
  if (sum(pos.mod$passed_filter,na.rm=TRUE)>0){ #lines that have 0 coverage are repoerted for all other metrics as NA which causes sum to return NA. na.rm only excludes these line from the sum rather than completely 
    motifs_sites_clean[i,3]<- 1
    #filter pos.mod to only include modified sites 
    pos.mod<-filter(pos.mod, passed_filter>0)
    #find the absolute difference from a dam site to a modified site
    ...
    wga_cov<-pos.mod[pos.min[p.value.min],"N_wga"]
    nat_cov<-pos.mod[pos.min[p.value.min],"N_nat"]
    motifs_sites_clean[i,5]<-wga_cov
    motifs_sites_clean[i,6]<-nat_cov
  }else {
    #dont filter pos.mod for modified sites but otherwise treat the same and report coverage. ? and take the average coverage for the exact match forward and reverse reads. 
    pos.abs<-abs(pos.mod$position-motifs_sites_clean[i,2])
    pos.min<-which(pos.abs==min(pos.abs))
    p.values<-pos.mod[pos.min,"u_test_pval_log"]
    p.value.min<-which(p.values==min(p.values))
...
write_delim(motifs_sites_clean,paste0("results/nanodisco/motif_pvals_with_coverage/",strain,"/",filenames,"_",motif,"_site_pvals.tab"),delim = "\t")
